...
export async function generateMetadata({ params }) {
  ...
  const { data: writer } = await sb.from('profiles').select('full_name, bio, avatar_url').eq('username', username).single()
...
export default async function WriterPage({ params }) {
  ...
  const { data: writer } = await sb.from('profiles').select('*').eq('username', username).single()
  if (!writer) notFound()
