...
import { getArticlesByCategory, getCategoryBySlug, getAllCategories, getActiveAds } from "@/lib/queries";
import { CATEGORY_DESCRIPTIONS } from "@/lib/category-copy";
...
export async function generateMetadata({
  ...
  const description =
    category.description || CATEGORY_DESCRIPTIONS[category.slug] || `Explore ${category.name} stories on Insight.`;
...
export default async function CategoryPage({ params }: { params: Promise<{ category: string }> }) {
  const { category: categorySlug } = await params;
  const [category, articles, allCategories, ads] = await Promise.all([
  ...
  const description =
    category.description || CATEGORY_DESCRIPTIONS[category.slug] || `Explore ${category.name} stories on Insight.`;
  const [lead, ...gridArticles] = articles;
