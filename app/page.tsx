...
export default async function HomePage() {
  const [posts, featured, ads, categories, categoryMap] = await Promise.all([
    getPublishedPosts(16),
    getFeaturedPosts(4),
    getActiveAds(),
    getAllCategories(),
    getCategoryMap()
  ...
  const heroImage = hero ? resolveCoverImage(hero) : null;
          ...
          <section className="hero" aria-labelledby="hero-title">
            <div className="hero-copy">
              <div className="hero-kicker">
                ...
                <span>{resolveCategoryName(hero, categoryMap)}</span>
              </div>
              ...
              <div className="hero-actions">
                ...
                <div className="story-meta hero-meta">
                  ...
                  <div>
                    ...
                    <span>{readTimeMinutes(hero.content)} min read</span>
                  </div>
                </div>
              </div>
            </div>
            {heroImage && (
              <div className="hero-visual">
                <Image src={heroImage} alt={hero.title} width={1536} height={1024} priority />
                <span className="hero-caption">{resolveCategoryName(hero, categoryMap)}</span>
        ...
        <section id="top-stories" className="section newsroom-section">
          ...
          <div className="newsroom-grid">
            <div className="news-column news-column-left">
              {secondary1 && <TextArticleCard post={secondary1} categoryMap={categoryMap} />}
              {secondary2 && <TextArticleCard post={secondary2} categoryMap={categoryMap} />}
            </div>

            <div className="news-column news-column-lead">
              {secondary3 && <LeadArticleCard post={secondary3} categoryMap={categoryMap} />}
              <div className="lead-news-subgrid">
                {secondary4 && <CompactArticleCard post={secondary4} categoryMap={categoryMap} />}
              </div>
            </div>
            ...
            <aside className="news-column news-column-opinion">
              ...
              {opinionPicks.map((post) => (
                <OpinionArticleCard key={post.id} post={post} categoryMap={categoryMap} />
              ))}
        ...
        {morePosts.length > 0 && (
          <section id="more-posts" className="section post-river-section">
            ...
            <div className="post-river-grid">
              {morePosts.map((post) => (
                <CompactArticleCard key={post.id} post={post} categoryMap={categoryMap} />
              ))}
            </div>
          </section>
        )}
