...
export default function Write() {
  ...
  useEffect(() => {
    window.uploadCoverFromModal = async (file) => {
      ...
      const { error } = await supabase.storage.from('place-images').upload(name, file, { contentType: file.type })
      if (error) { toast.error('Upload failed'); setUploadingCover(false); return }
      const { data: { publicUrl } } = supabase.storage.from('place-images').getPublicUrl(name)
