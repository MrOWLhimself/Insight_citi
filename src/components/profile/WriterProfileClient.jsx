...
function Profile({ writer, articles, username }) {
  ...
  const { data: followCheck } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase?.from('user_follows').select('id').eq('following_id', writer.id).eq('follower_id', user.id).single()
      return data
    },
  ...
  const toggleFollow = useMutation({
    mutationFn: async () => {
      ...
      if (followCheck) await supabase?.from('user_follows').delete().eq('id', followCheck.id)
      else await supabase?.from('user_follows').insert({ following_id: writer.id, follower_id: user.id })
    },
